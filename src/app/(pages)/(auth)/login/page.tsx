    ...
    const handleLogin = (e: any) => {
        ...
        dispatch(login(loginForm)).then((res: any) => {
            if (res.payload.meta.code == StatusCodes.OK) {
                ...
                toast.error(
                    `${res.payload.meta.message}. ${res.payload.result.message}`
                );
            }
        });
    };
    ...
    const googleLogin = useGoogleLogin({
        onSuccess: async (tokenResponse: TokenResponse) => {
            dispatch(authGoogle(tokenResponse)).then((res: any) => {
                if (
                    res.payload?.meta?.code == StatusCodes.OK ||
                    res.status == StatusCodes.OK
                ) {
                    if (
                        res.payload.meta.message == STATUS_SIGNIN.Authenticated
                    ) {
                        router.push("/dashboard");
    ...
    return (
        <>
            <div className="grid grid-cols-1 sm:grid-cols-3 h-screen w-full">
                <div className="bg-custom-green-primary col-span-2 flex flex-col justify-center"></div>
                            ...
                            <button
                                className={`w-full my-5 py-2 ${
                                    auth.isLoading
                                        ...
                                        : "bg-custom-green-primary shadow-md"
                                } hover:bg-custom-green-dark text-white font-semibold rounded-lg`}
